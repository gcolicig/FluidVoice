...
final class SettingsStore: ObservableObject {
    ...
    enum SpeechModel: String, CaseIterable, Identifiable, Codable {
        ...
        var displayName: String {
            ...
            case .whisperSwissGermanQ8: return "Whisper Swiss German (Q8)"
            }
        }
        ...
        var downloadSize: String {
            ...
            case .whisperSwissGermanQ8: return "~1.67 GB"
            }
        }
        ...
        var expectedDownloadBytes: Int64 {
            ...
            case .whisperSwissGermanQ8: return 1_668_741_504
            }
        }
        ...
        var isWhisperModel: Bool {
            switch self {
            case .parakeetTDT, .parakeetTDTv2, .parakeetRealtime, .qwen3Asr, .cohereTranscribeSixBit, .nemotronOffline, .nemotronStreaming, .nemotronStreaming320, .appleSpeech, .appleSpeechAnalyzer, .customOpenAICompatible: return false
            default: return true
            }
        }
        ...
        var whisperModelFile: String? {
            ...
            case .whisperSwissGermanQ8: return "whisper-large-v3-swiss-german-Q8_0.gguf"
            default: return nil
            }
        }
        ...
        var whisperForcedLanguage: String? {
            switch self {
            case .whisperSwissGermanQ8: return "de"
            default: return nil
            }
        }
        ...
        var whisperModelDownloadOverrideURL: URL? {
            ...
            case .whisperSwissGermanQ8:
                return URL(
                    string: "https://huggingface.co/gcoli/whisper-large-v3-swiss-german-gguf-q8_0/resolve/main/whisper-large-v3-swiss-german-Q8_0.gguf"
                )
            default:
                return nil
            }
        ...
        static let legacyWhisperModelFiles: Set<String> = [
            // Superseded Swiss German preview quantization; auto-removed so the
            // orphaned 900 MB file does not linger after the Q8 switch.
            "whisper-large-v3-swiss-german-Q4_0.gguf",
            "ggml-tiny.bin",
            "ggml-base.bin",
            "ggml-small.bin",
            "ggml-medium.bin",
        ...
        static var availableModels: [SpeechModel] {
            allCases.filter { model in
                ...
                if model == .whisperSwissGermanQ8, !CPUArchitecture.isAppleSilicon {
                    return false
                }
        ...
        var humanReadableName: String {
            ...
            case .whisperSwissGermanQ8: return "Schweizerdeutsch Offline"
            }
        }
        ...
        var cardDescription: String {
            ...
            case .whisperSwissGermanQ8:
                return "Whisper Large v3 fine-tuned for Swiss German dialects, quantized to Q8 and running fully offline. Also serves as the live-preview engine for the Custom Server model."
            }
        }
        ...
        var speedRating: Int {
            ...
            case .whisperSwissGermanQ8: return 3
            }
        }
        ...
        var accuracyRating: Int {
            ...
            case .whisperSwissGermanQ8: return 5
            }
        }
        ...
        var speedPercent: Double {
            ...
            case .whisperSwissGermanQ8: return 0.65
            }
        }
        ...
        var accuracyPercent: Double {
            ...
            case .whisperSwissGermanQ8: return 0.90
            }
        }
        ...
        var supportsStreaming: Bool {
            switch self {
            case .qwen3Asr, .whisperMedium, .whisperLargeTurbo, .whisperLarge, .whisperSwissGermanQ8:
                return false // Too slow for real-time chunk processing
            case .customOpenAICompatible:
                // Final transcription stays one HTTP round-trip; the live preview
                // runs on the local Swiss German Q8 model when it is installed.
        ...
        var streamingPreviewIntervalSeconds: Double {
            ...
            case .cohereTranscribeSixBit:
                return 1.0
            case .customOpenAICompatible:
                return 1.0 // Local large-Q8 preview passes need more headroom than 0.6s
            default:
                return 0.6
            }
        }
        ...
        var provider: Provider {
            ...
            case .whisperTiny, .whisperBase, .whisperSmall, .whisperMedium, .whisperLargeTurbo, .whisperLarge:
                return .openai
            case .customOpenAICompatible, .whisperSwissGermanQ8:
                return .custom
            }
        }
        ...
        var isInstalled: Bool {
            switch self {
            case .appleSpeech, .appleSpeechAnalyzer, .customOpenAICompatible:
                return true
            case .parakeetTDT:
                #if canImport(FluidAudio)
...
extension SettingsStore {
    ...
    // MARK: - Custom OpenAI-compatible ASR Server

    /// Keychain provider ID for the custom ASR server API key.
    static let customASRKeychainProviderID = "custom-asr"

    /// Base URL of the OpenAI-compatible transcription server (e.g. a local oMLX instance).
    var customASRBaseURL: String {
        get {
            self.defaults.string(forKey: Keys.customASRBaseURL) ?? "http://127.0.0.1:8888/v1"
        }
        set {
            objectWillChange.send()
            self.defaults.set(newValue, forKey: Keys.customASRBaseURL)
        }
    }

    /// Model name sent in the `model` field of transcription requests.
    var customASRModelName: String {
        ...
        set {
            objectWillChange.send()
            self.defaults.set(newValue, forKey: Keys.customASRModelName)
        }
    }

    /// Optional ISO-639-1 language hint (empty = server auto-detects).
    var customASRLanguage: String {
        get {
            self.defaults.string(forKey: Keys.customASRLanguage) ?? ""
        }
        set {
            objectWillChange.send()
    ...
    /// powered by the local Swiss German Q8 model. Off keeps the GGUF on disk
    ...
    /// API key for the custom ASR server, stored in the Keychain.
    var customASRAPIKey: String {
        get {
            (try? self.keychain.fetchKey(for: Self.customASRKeychainProviderID)) ?? ""
        }
        set {
            objectWillChange.send()
            let trimmed = newValue.trimmingCharacters(in: .whitespacesAndNewlines)
            if trimmed.isEmpty {
                try? self.keychain.deleteKey(for: Self.customASRKeychainProviderID)
            } else {
                try? self.keychain.storeKey(trimmed, for: Self.customASRKeychainProviderID)
            }
        }
    }
